...
class App extends Component {
  ...
  overlordCountLeft = 0;
  overlordMultiplierLeft = 0;
  overlordCountRight = 0;
  overlordMultiplierRight = 0;
  ...
  setGlobalStats = (side) => {
    ...
    *this[`mana${side}`]*this[`archangel${side}`] > itemStats.ad + appliedStats.ad + (itemStats.mana 
    ...
    + (itemStats.hp + runeStats.hp + appliedStats.hp + (itemStats.mana + appliedStats.mana)
    *this[`winterMultiplier${side}`]*this[`mana${side}`])*this[`overlordMultiplier${side}`]
            ...
            this[`applied${x}${side}`][y].ratioPer.perStatTypes.map((z, i) => {
              if (z === 'hp') {
                appliedStats[y] += this[`applied${x}${side}`][y].ratioPer.perQuantityValues[i] * (itemStats.hp
                  + runeStats.hp + appliedStats.hp + (itemStats.mana + appliedStats.mana)
                  *this[`winterMultiplier${side}`]*this[`mana${side}`])
    ...
    this[`bonusStats${side}`] = {
      ...this[`bonusStats${side}`],
      hp: itemStats.hp + runeStats.hp + appliedStats.hp + (itemStats.mana + appliedStats.mana)
      *this[`winterMultiplier${side}`]*this[`mana${side}`],
      ...
      if (this[`champName${side}`] === 'Kled') {
        ...
        this.setState(prevState => ({
          [`tfTotalStats${side}`]: {
            ...
            appliedStats.hp + (itemStats.mana + appliedStats.mana)*this[`winterMultiplier${side}`]*this[`mana${side}`]
          }
        }))
      }
    };
  ...
  calculateAbility(side) {
    ...
    for (var i = 0; i < this.abilities.length; i++) {
      ...
      function mapValuesToAAFunctions (statType, path, toCalc, statText) {
        ...
        if (path["flatMin"]) {
          if (toCalc) {
            applyAbility(statType, 'flatMin', path['flatMin'], 'minMax')
            minCounter += arrayCheck(path['flatMin'])
          } else {
            colorMin('Min: ');
        ...
        if (path["flatMax"]) {
          if (toCalc) {
            applyAbility(statType, 'flatMax', path['flatMax'], 'minMax')
            maxCounter += arrayCheck(path['flatMax'])
          } else {
            singleBreak();
            colorMin('Max: ');
            addText(removeParen(path['flatMax']))
          }
        };
        if (path['ratio']) {
          if (toCalc) {
        ...
        if (path['byLvl']) {
          if (toCalc) {
            ...
            statCounter += path['byLvl'][champLevel]
          } else {
        ...
        if (path["byLvlMin"]) {
          if (toCalc) {
            applyAbility(statType, 'byLvlMin', path['byLvlMin'], 'minMax');
            minCounter += path['byLvlMin'][champLevel]
          } else {
            colorMin('Min: ');
            addText(path['byLvlMin'][0] + ' to ' + path['byLvlMin'][17] + ', based on lvl. ');
            underLine('Currently');
            addText(path['byLvlMin'][champLevel])
          }
        };
        if (path["byLvlMax"]) {
          if (toCalc) {
            applyAbility(statType, 'byLvlMax', path['byLvlMax'], 'minMax');
            maxCounter += path['byLvlMax'][champLevel]
          } else {
            singleBreak();
            colorMin('Max: ');
            addText(path['byLvlMax'][0] + ' to ' + path['byLvlMax'][17] + ', based on lvl. ');
            underLine('Currently');
            addText(path['byLvlMax'][champLevel])
          }
        };
        if (path["APRatio"]) {
          if (toCalc) {
      ...
      function listOrCalcValues(path) {
        if (ability === 'passive' || document.getElementById(`${ability}Rank${side}`).value == 0) {
          if (path["dmg"]) {
          ...
          if (path["totalDmgRatio"]) {
            colorOrchid('Total Damage Ratio');
            removeSpace(path["totalDmgRatio"]);
          };
          if (path["dmgByWRank"]) {
            addText('[+' + path["dmgByWRank"][0] + " to " + path["dmgByWRank"][4] + " Damage, based on W rank. ");
            underLine("Currently");
            addText(path["dmgByWRank"][WRank] + '] ');
          };
          if (path["dmgByERank"]) {
            addText('[' + path["dmgByERank"][0] + " to " + path["dmgByERank"][4] + ", based on E rank. ");
            underLine("Currently");
            addText(path["dmgByERank"][ERank] + '] ');
          };
          [["APRatio", colorAP, ' AP'], ["ADRatio", colorAD, ' AD'], 
          ["dmgPerLethality", colorArPen, ' Lethality'],["missingHPRatio", colorHP, " Missing HP"],
          ...
          [["maxHPRatioPer100BonusAD", ' Max HP', colorAD, '100 Bonus AD'],
          ['missingHPRatioPer100AP', ' Missing HP', colorAP, '100 AP'],
          ...
          ['enemyMaxHPRatioPerBonusAS', ' Enemy Max HP', colorAS, ' Bonus AS'],
          ...
          [["ADRatioByLvl", colorAD, ' AD'], ["maxHPRatioByLvl", colorHP, ' Max HP'],
          ["enemyMaxHPRatioByLvl", colorHP, ' Enemy Max HP'],
          ['APRatioByLvl', colorAP, ' AP'],["enemyCurrentHPRatioByLvl", colorHP, ' Enemy Current HP'],
          ["enemyMissingHPRatioByLvl", colorHP, ' Enemy Missing HP']].map(x => {
            if (path[x[0]]) {
              addText('(+' + path[x[0]][0] + " to " + path[x[0]][17]);
              x[1](x[2]); 
              addText("ratio, based on lvl. ");
              underLine('Currently');
              addText(path[x[0]][champLevel] + ")");
            }
          });
          if (path["bonusMoveSpeedRatioByLvl"]) {
            addText('(' + path["bonusMoveSpeedRatioByLvl"][0] + " to " 
            + path["bonusMoveSpeedRatioByLvl"][17] + " Bonus Move Speed");
            prependIcon(moveSpeedIcon);
            addText("ratio, based on lvl. ");
            underLine('Currently');
            addText(path["bonusMoveSpeedRatioByLvl"][champLevel] + ") ")
          };
          if (path["bonusADPerAttackSpeedRatio"]) {
            addText(' (+' + path["bonusADPerAttackSpeedRatio"]);
            colorAD(" Bonus AD");
            addText("ratio per");
            colorAS(' AS');
            addText(')');
          };
          if (path["enemyCurrentHPRatioPerStack"]) {
            addText(' (+' + removeParen(path["enemyCurrentHPRatioPerStack"]));
          ...
          if (path["enemyMaxHPRatioPerStack"]) {
            addText(" (+" + removeParen(path["enemyMaxHPRatioPerStack"]));
            colorHP(' Enemy Max HP');
            addText("ratio per stack)");
          };
          if (path["enemyMissingHPRatioPerStack"]) {
            addText(' (+' + path["enemyMissingHPRatioPerStack"]);
            colorHP(" Enemy Missing HP");
            addText("ratio per stack)");
          };
          if (path['enemyResistRatio']) {
            addText(' (+' + path['enemyResistRatio']);
            colorArmor(' Enemy Armor');
            addText('and');
            colorMR(' Magic Resist');
            addText('ratio)');
          };
          if (path["ADRatioPerCritChance"]) {
            addText(" (+" + path["ADRatioPerCritChance"]);
            colorAD(" AD");
            addText('ratio per');
            colorCrit(" Crit Chance");
            if (path["ADRatioPerCritChanceWithIE"]) {
              addText('. With ')
              addIE();
              addText(' IE: ' + path["ADRatioPerCritChanceWithIE"]);
              colorAD(" AD");
              addText('ratio');
            }; 
            addText(')');
            if (ability === 'passive') {
              var dmgCounter = 0;
              var IECounter = 0;
              dmgCounter += path["ADRatioPerCritChance"] * totalCritChance * totalAD;
              if (path["ADRatioByLvl"]) {
                dmgCounter += path["ADRatioByLvl"][champLevel] * totalAD;
              };
              doubleBreak();
              underLine('Current Damage');
              addText(Math.round(dmgCounter));
              if (path["ADRatioPerCritChanceWithIE"]) {
                IECounter += path["ADRatioPerCritChanceWithIE"] * totalCritChance * totalAD;
          ...
          if (path["dmgPerStack"]) {
            addText(" (+" + path["dmgPerStack"] + ' per stack)');
          };
        } else {
      ...
      function minMaxListOrCalcValues(path, prefix) {
        if (ability === 'passive' || document.getElementById(`${ability}Rank${side}`).value == 0) {
          if (path[`${prefix}Dmg`]) {
            removeSpace(path[`${prefix}Dmg`]);
          };
          if (path[`${prefix}DmgByLvl`]) {
            if (path[`${prefix}Dmg`]) {
              addText(' [+')
            } else {
              addText('[')
            };
            addText(path[`${prefix}DmgByLvl`][0] + " to " + path[`${prefix}DmgByLvl`][17] + ", based on lvl. ");
            underLine('Currently');
            addText(path[`${prefix}DmgByLvl`][champLevel] + ']')
          };
          if (path[`${prefix}DmgByWRank`]) {
            addText('[+' + path[`${prefix}DmgByWRank`][0] + " to " + path[`${prefix}DmgByWRank`][4] 
            + " Damage, based on W rank. ");
            underLine("Currently");
            addText(path[`${prefix}DmgByWRank`][WRank] + '] ');
          };
          if (path[`${prefix}DmgByERank`]) {
            addText('[' + path[`${prefix}DmgByERank`][0] + " to " + path[`${prefix}DmgByERank`][4] 
            + ", based on E rank. ");
            underLine("Currently");
            addText(path[`${prefix}DmgByERank`][ERank] + '] ');
          };
          if (path[`${prefix}TotalDmgRatio`]) {
            colorOrchid('Total Damage Ratio');
          ...
          [`${prefix}MissingHPRatio`, colorHP, ' Missing HP'],
          ...
          [[`${prefix}MaxHPRatioPer100BonusAD`, ' Max HP', colorAD, '100 Bonus AD'],
          [`${prefix}MissingHPRatioPer100AP`, ' Missing HP', colorAP, '100 AP'],
          ...
          [`${prefix}EnemyMaxHPRatioPerBonusAS`, ' Enemy Max HP', colorAS, ' Bonus AS'],
          ...
          [[`${prefix}ADRatioByLvl`, colorAD, ' AD'], [`${prefix}APRatioByLvl`, colorAP, ' AP'],
          [`${prefix}MaxHPRatioByLvl`, colorHP, ' Max HP'],
          [`${prefix}EnemyMaxHPRatioByLvl`, colorHP, ' Enemy Max HP'],
          [`${prefix}EnemyCurrentHPRatioByLvl`, colorHP, ' Enemy Current HP'],
          [`${prefix}EnemyMissingHPRatioByLvl`, colorHP, ' Enemy Missing HP']].map(x => {
            if (path[x[0]]) {
              addText('(+' + path[x[0]][0] + " to " + path[x[0]][17]);
              x[1](x[2]); 
              addText("ratio, based on lvl. ");
          ...
          if (path[`${prefix}BonusMoveSpeedRatioByLvl`]) {
            addText('(' + path[`${prefix}BonusMoveSpeedRatioByLvl`][0] + " to " 
            + path[`${prefix}BonusMoveSpeedRatioByLvl`][17] + " Bonus Move Speed");
            prependIcon(moveSpeedIcon);
            addText("ratio, based on lvl. ");
            underLine('Currently');
            addText(path[`${prefix}BonusMoveSpeedRatioByLvl`][champLevel] + ") ")
          };
          if (path[`${prefix}BonusADPerAttackSpeedRatio`]) {
            addText(' (+' + path[`${prefix}BonusADPerAttackSpeedRatio`]);
            colorAD(" Bonus AD");
            addText("ratio per");
            colorAS(' AS');
            addText(')');
          };
          if (path[`${prefix}EnemyCurrentHPRatioPerStack`]) {
            addText(' (+' + removeParen(path[`${prefix}EnemyCurrentHPRatioPerStack`]));
            colorHP(" Enemy Current HP");
            addText("ratio per stack)");
          };
          if (path[`${prefix}EnemyMaxHPRatioPerStack`]) {
            addText(" (+" + removeParen(path[`${prefix}EnemyMaxHPRatioPerStack`]));
            colorHP(' Enemy Max HP');
            addText("ratio per stack)");
          };
          if (path[`${prefix}EnemyMissingHPRatioPerStack`]) {
            addText(' (+' + path[`${prefix}EnemyMissingHPRatioPerStack`]);
            colorHP(" Enemy Missing HP");
            addText("ratio per stack)");
          };
          if (path[`${prefix}EnemyResistRatio`]) {
            addText(' (+' + path[`${prefix}EnemyResistRatio`]);
            colorArmor(' Enemy Armor');
            addText('and');
            colorMR(' Magic Resist');
            addText('ratio)');
          };
          if (path[`${prefix}ADRatioPerCritChance`]) {
            addText(" (+" + path[`${prefix}ADRatioPerCritChance`]);
            colorAD(" AD");
            addText('ratio per');
            colorCrit(" Crit Chance");
            if (path[`${prefix}ADRatioPerCritChanceWithIE`]) {
              addText('. With ')
              addIE();
              addText(' IE: ' + path[`${prefix}ADRatioPerCritChanceWithIE`]);
              colorAD(" AD");
              addText('ratio');
            }; 
            addText(')');
            if (ability === 'passive') {
              var dmgCounter = 0;
              var IECounter = 0;
              dmgCounter += path[`${prefix}ADRatioPerCritChance`] * totalCritChance * totalAD;
              if (path[`${prefix}ADRatioByLvl`]) {
                dmgCounter += path[`${prefix}ADRatioByLvl`][champLevel] * totalAD;
              };
              doubleBreak();
              underLine('Current Damage');
              addText(Math.round(dmgCounter));
              if (path[`${prefix}ADRatioPerCritChanceWithIE`]) {
                IECounter += path[`${prefix}ADRatioPerCritChanceWithIE`] * totalCritChance * totalAD;
                addText(' (' + Math.round(IECounter + dmgCounter) + ' with ');
                addIE();
                addText(' IE)');
              };
          ...
          if (path[`${prefix}BonusDmgRatioPerCritChance`]) {
            ...
            if (path[`${prefix}BonusDmgRatioPerCritChanceWithIE`]) {
              addText('(' + path[`${prefix}BonusDmgRatioPerCritChanceWithIE`] + ' with ');
              addIE();
              addText(' IE)');
            }
          };
      ...
      if (champFile[ability]) {
        if (ability === 'passive' || document.getElementById(`${ability}Rank${side}`).value == 0) {
          ...
          if (champFile[ability]["toggleStats"]) {
            ...
            if (path["armorPenRatio"]) {
              ...
              mapValuesToAAFunctions('arPenRatio', path["armorPenRatio"], false, "Armor Penetration Ratio")
            };
            if (path["armor"]) {
              prependIcon(armorIcon);
          ...
          if (champFile[ability]["autoEmpower"]) {
            ...
            if (damage["system"] === "minMax" ) {
              ...
              minMaxListOrCalcValues(damage, "max");
            ...
            if (damage["system"] === "minMax" ) {
              ...
              if (damage["medDmg"]) {
                ...
                if (damage["medBonusADRatio"]) {
                  addText(" (+" + removeParen(damage["medBonusADRatio"]));
                  colorAD(" Bonus AD");
                  addText("ratio)");
                };
                singleBreak();
            ...
            if (damage["system"] === "2Part" || damage["system"] === "3Part") {
              (function() {
                ...
                if (part["system"] === "minMax" ) {
                  ...
                  minMaxListOrCalcValues(part, "min");

                  singleBreak();
                  colorMin('Max: ');

                  minMaxListOrCalcValues(part, "max");
                };
          ...
          if (champFile[ability]["tickDamage"]) {
            ...
            listOrCalcValues(path);
        
            if (path["system"] === "minMax" ) {
              colorMin('Min: ');
              
              minMaxListOrCalcValues(path, "min");
              ...
              minMaxListOrCalcValues(path, "max");
            };
        ...
        if (ability !== 'passive' && document.getElementById(`${ability}Rank${side}`).value > 0 ) {
          ...
          if (champFile[ability]["toggleStats"]) {
            ...
            if (path["armorPenRatio"]) {
              prependIcon(arPenIcon);
              mapValuesToAAFunctions('arPenRatio', path["armorPenRatio"], true, "Armor Penetration Ratio")
            };
            if (path["armor"]) {
              prependIcon(armorIcon);
          ...
          if (champFile[ability]["damage"]) {
            ...
            if (totalDmgCount !== 0) {
              if (damage["bonusDmgRatioPerCritChance"]) {
                totalDmgCount *= (1 + damage["bonusDmgRatioPerCritChance"] * totalCritChance*100);
              };
            ...
            if (damage["system"] === "2Part" || damage["system"] === "3Part") {
              (function() {
                ...
                if (part["enemyMaxHPRatioPer100BonusAD"] && enemyStats.baseHP) {
                  partDmgCount += arrayCheck(part["enemyMaxHPRatioPer100BonusAD"]) * enemyTotalHP
                  * bonusAD/100;
                };
                ...
                var maxHPRatioCounter = 0;
                if (part["enemyMaxHPRatio"] && !enemyStats.baseHP) {
                  maxHPRatioCounter += arrayCheck(part["enemyMaxHPRatio"]);
                };
                if (part["enemyMaxHPRatioPer100BonusAD"] && !enemyStats.baseHP) {
                  maxHPRatioCounter += arrayCheck(part["enemyMaxHPRatioPer100BonusAD"])*bonusAD/100
                };
                if (maxHPRatioCounter !== 0) {
                  addText(" (+" + maxHPRatioCounter.toFixed(3));
                  colorHP(' Enemy Max HP');
                  addText('ratio)');
                };
                if (part["enemyCurrentHPRatio"] && !enemyStats.baseHP) {
                ...
                if (part["system"] === "minMax" ) {
                  ...
                  if (part["bonusDmgRatioPerCritChance"]) {
                    minPartDmgCount *= (1 + part["bonusDmgRatioPerCritChance"] * totalCritChance*100);
                  };
                  ...
                  if (part["bonusDmgRatioPerCritChance"]) {
                    maxPartDmgCount *= (1 + part["bonusDmgRatioPerCritChance"] * totalCritChance*100);
                  };
            ...
            if (path["system"] === "minMax" ) {
              ...
              if (path["bonusDmgRatioPerCritChance"]) {
                if (path["bonusDmgRatioPerCritChanceWithIE"] && this[`ieCount${side}`]) {
                  minTickDmgCount *= (1 + path["bonusDmgRatioPerCritChanceWithIE"] * totalCritChance*100);
                } else {
                  minTickDmgCount *= (1 + path["bonusDmgRatioPerCritChance"] * totalCritChance*100);
                }
              };
              ...
              if (path["bonusDmgRatioPerCritChance"]) {
                if (path["bonusDmgRatioPerCritChanceWithIE"] && this[`ieCount${side}`]) {
                  maxTickDmgCount *= (1 + path["bonusDmgRatioPerCritChanceWithIE"] * totalCritChance*100);
                } else {
                  maxTickDmgCount *= (1 + path["bonusDmgRatioPerCritChance"] * totalCritChance*100);
                }
              };
  ...
  legendItems = {
    ...
    <div>
      ...
      <span>
      Enemy champions within 700 units of you become cursed, reducing their magic 
      resistance by 30%. Gain 10 bonus magic resistance per cursed enemy.
      </span>
    </div>,
    1:
    <div>
    ...
    <div>
      ...
      <span>
      Dealing physical damage to an enemy champion applies a stack of Carve for 6 seconds, 
      stacking up to 5 times. Each stack reduces the target's armor by 6%, up to 30%.
      </span>
      <hr></hr>
    ...
    6:
    <div>
      <b className='yellow'>Blackfire Torch</b>
      <hr></hr>
      <span>
      ...
      <hr></hr>
      <i className='yellow'>Unique Passive - Baleful Blaze: </i>
      <span>
      Dealing ability damage burns enemies, dealing 7.5 (+ 1.5% AP) magic damage every 0.5 seconds 
      over 3 seconds, for a total of 45 (+ 9% AP). Each tick deals 10 bonus damage against monsters.
      </span>
      <hr></hr>
      <i className='yellow'>Unique Passive - Blackfire: </i>
      <span>
      For each champion, epic monster, and large monster afflicted with Baleful Blaze's burn, 
      increase your ability power by 4%.
      </span>
    </div>,
    7:
    <div>
      ...
      <span>
      Basic attacks deal (Melee 10% / Ranged 6%) of the target's current health bonus 
      physical damage on-hit, with a minimum of 15 against all units.
      </span>
      <hr></hr>
      <i className='yellow'>Unique Passive - Clawing Shadows: </i>
      <span>
      Basic attacks on-hit against enemy champions apply a stack for 6 seconds, stacking up 
      to 3 times. The third stack consumes them all to slow the target by 30% for 1 second.
      </span>
      <br></br>
      <i className='yellow'>Cooldown: </i>
      <span>
    ...
    <div>
      ...
      <hr></hr>
      <span>
        80 <img src={ADIcon}></img> Attack Damage
      </span>
      <br></br>
      <span>
        15% <img src={lifestealIcon}></img> Life Steal
      </span>
      <hr></hr>
      <i className='yellow'>Unique Passive - Ichorshield: </i>
      <span>
      Convert the healing received from life steal in excess of maximum health into a shield 
      for up to 165 - 315 (based on level), which lasts until destroyed.
      </span>
    </div>,
    10:
    <div>
    ...
    <div>
      ...
      Reduce incoming champion damage by (Melee 35% / Ranged 25%) for 2 seconds after taking damage from
       a champion. Then unleash a shockwave around you that slows nearby enemies by 50% for 1.5 seconds.
      </span>
      <br></br>
      <i className='yellow'>Cooldown: </i>
      <span>
    ...
    <div>
      ...
      <span>
      Dealing ability damage to an enemy champion grants you 40-60 (based on level) bonus
    ...
    <div>
      ...
      <br></br>
      <span>
        16% <img src={healShieldIcon}></img> Heal and Shield Power
      </span>
      <br></br>
      <span>
      ...
      <span>
      Gain 2% heal and shield power and 10 AP for every additional 100% base mana regeneration.
      </span>
    </div>,
    16:
    <div>
    ...
    <div>
      ...
      If an enemy champion dies within 3 seconds of you damaging them, remove Ignore Pain's remaining
       stored damage and heal for 75% bonus AD over 2 seconds.
      </span>
    </div>,
    18:
    <div>
    ...
    <div>
      ...
      <span>
      Damaging an enemy champion with a basic attack or ability damage grants a Soul Shard, up to 2.
       Healing or shielding an allied champion (excluding self) consumes all Soul Shards to heal them
        for 75 per shard and deal 60 magic damage per shard to the nearest enemy champion within 1100 units of the ally.
      </span>
    </div>,
    20:
    <div>
      ...
      Hitting an enemy champion with 2 separate attacks or abilities within 2 seconds deals bonus physical
       damage equal to (Melee 6% / Ranged 4%) of target's maximum health and grants you a shield for
    ...
    <div>
      ...
      <br></br>
      <span>
        25% <img src={critChanceIcon}></img> Critical Strike Chance
      </span>
      <hr></hr>
      <i className='yellow'>Unique Passive - Essence Drain: </i>
      <span>
    ...
    <div>
      ...
      a shield that absorbs (Melee 400-700 / Ranged 320-560) (based on level) damage for 3 seconds.
      </span>
      <br></br>
      <i className='yellow'>Cooldown: </i>
      <span>
    ...
    <div>
      ...
      <span>
      While your Worthy ally is within 1250 range and you are above 30% of your maximum health, 
      redirect 12% of the pre-mitigation physical and magic damage they take to you as the respective 
      damage type and heal for 10% of the post-mitigation damage dealt by your Worthy ally to champions.
      </span>
    </div>,
    42:
    <div>
      ...
      <span>
      Every third basic attack deals (Melee 140-310 / Ranged 112-248) (based on level) bonus physical 
      damage on-hit, increased by up to 50% based on target's missing health. Stacks last 3 seconds.
      </span>
    </div>,
    43:
    <div>
    ...
    <div>
      ...
      <span>
      Dealing ability damage to an enemy consumes all 6 Shot Charges to deal 60 (+4% AP) bonus magic 
      ...
      deal an additional 30-150 (based on remaining Shot Charges) (+2%-10% AP) magic damage 
      to the primary target, for a total of 90-210 (+6%-14% AP).
      </span>
      <br></br>
      <i className='yellow'>Cooldown: </i>
      <span>
    ...
    <div>
      ...
      magic damage for 3 seconds. Grants 10% omnivamp until end of combat.
      </span>
      <br></br>
      <i className='yellow'>Cooldown: </i>
      <span>
    ...
    59:
    <div>
      <b className='yellow'>Navori Flickerblade</b>
      <hr></hr>
      <span>
        40% <img src={attackSpeedIcon}></img> Attack Speed
      </span>
    ...
    <div>
      ...
      <span>
      After being out-of-combat with enemy champions for 8 seconds, gain (Melee 5-10 / Ranged 3-6) (based 
      on level) lethality. This bonus remains for 3 seconds after dealing damage to an enemy champion.
      </span>
      <hr></hr>
      <i className='yellow'>Unique Passive - Extraction: </i>
      <span>
      Damaging an enemy champion within 3 seconds of takedown grants you 200 bonus 
      movement speed that decays over 1.5 seconds.
      </span>
    </div>,
    61:
    <div>
      <b className='yellow'>Overlord's Bloodmail</b>
      <hr></hr>
      <span>
      ...
      <hr></hr>
      <i className='yellow'>Unique Passive - Tyranny: </i>
      <span>
      Gain 2% of bonus health as bonus attack damage.
      </span>
      <hr></hr>
      <i className='yellow'>Unique Passive - Retribution: </i>
      <span>
      Gain 0% - 10% (based on missing health) bonus attack damage.
      </span>
    </div>,
    62:
    <div>
      ...
      <hr></hr>
      <span>
        60% <img src={attackSpeedIcon}></img> Attack Speed
      </span>
      <br></br>
      <span>
        25% <img src={critChanceIcon}></img> Critical Strike Chance
      ...
      <br></br>
      <span>
        12% <img src={moveSpeedIcon}></img> Movement Speed
      </span>
      <hr></hr>
      <i className='yellow'>Unique Passive - Spectral Waltz: </i>
      <span>
      Become permanently ghosted.
      </span>
    </div>,
    63:
    <div>
    ...
    <div>
      ...
      <br></br>
      <span>
        75 <img src={armorIcon}></img> Armor
      </span>
      <hr></hr>
      <i className='yellow'>Unique Active - Humility: </i>
      <span>
      Unleash a 500 range shockwave around you that slows nearby enemies by 70% for 2 seconds.
      </span>
      <br></br>
      <i className='yellow'>Cooldown: </i>
      <span>
    ...
    <div>
      ...
      <span>
      Basic attacks fire additional bolts at up to 2 enemies in front of you, each dealing 55% AD physical 
    ...
    <div>
      ...
      <hr></hr>
      <span>
        115 <img src={APIcon}></img> Ability Power
      </span>
      <br></br>
      <span>
        15 <img src={mPenIcon}></img> Magic Penetration
      </span>
      <hr></hr>
      <i className='yellow'>Unique Passive - Cinderbloom: </i>
      <span>
      Magic damage and true damage critically strike enemies below 40% maximum health for 120% 
      damage, increased to 125% for damage over time effects and damage dealt by pets.
      </span>
    </div>,
    77:
    <div>
    ...
    <div>
      ...
      <hr></hr>
      <i className='yellow'>Unique Passive - Electrospark: </i>
      <span>
      Upon killing a minion or monster, unleash an electric spark at the location of their death 
      which chains to the closest target within 500 units, up to 6 targets, dealing 120 magic damage to each. 
      </span>
      <br></br>
      <i className='yellow'>Cooldown: </i>
      <span>
      ...
      <span>
      Scoring a takedown against an enemy champion unleashes a lightning shock at the location of their 
      death which chains to the closest target within 650 units, repeating from the new one to strike 
      each subseqeuent target, dealing 270 magic damage to each.
      </span>
    </div>,
    83:
    <div>
    ...
    <div>
      ...
      <span>
      Dealing damage to an enemy champion equal to 25% of their maximum health within 2.5 seconds 
      inflicts them with Squall and grants you 35% bonus movement speed for 2 seconds.
      </span>
      <br></br>
      <i className='yellow'>Cooldown: </i>
      <span>
      ...
      140 (+20% AP) magic damage to them. If the target dies before being struck, they emit an 
      electric field instantly that shocks all enemy champions 
    ...
    <div>
      ...
      and heal you for (Melee 120% / Ranged 60% base AD) (+6% of your missing health). Excess healing 
      beyond maximum health is converted to bonus health for 8 seconds.
      </span>
      <br></br>
      <i className='yellow'>Cooldown: </i>
      <span>
    ...
    <div>
      ...
      damage by 10%, stacking up to 60%.
      </span>
    </div>,
    88:
    <div>
    ...
    <div>
      ...
      <span>
      Every 5 seconds in combat with champions, deal 30-50 (based on level) (+3% bonus health) magic 
    ...
    <div>
      ...
      <hr></hr>
      <span>
        1000 <img src={healthIcon}></img> Health
      </span>
      <br></br>
      <span>
        100% <img src={healthRegenIcon}></img> Base Health Regen.
      ...
      <span>
      If you have at least 1500 bonus health and have not taken damage in the last 6 seconds 
      (3 seconds for damage from non-champions), regenerate 2.5% maximum health every 0.5 
      seconds and gain 5% bonus movement speed.
      </span>
    </div>,
    100:
    <div>
    ...
    <div>
      ...
      <span>
      Basic attacks deal 40-80 (based on level) bonus magic damage on-hit.
      </span>
    </div>,
    102:
    <div>
    ...
    103:
    <div>
      <b className='yellow'>Yun Tal Wildarrows</b>
      <hr></hr>
      <span>
        65 <img src={ADIcon}></img> Attack Damage
      </span>
      ...
      <hr></hr>
      <i className='yellow'>Unique Passive - Serrated Edge: </i>
      <span>
      Basic damage that critically strikes on-hit inflicts a bleed on the target, causing them to 
      take 8.75% AD bonus physical damage every 0.5 seconds over 2 seconds. Subsequent 
  ...
  onLegendClick = (event, side, legendIndex) => {
    ...
    if (itemTitle === `Overlord's Bloodmail`) {
      if (this[`overlordCount${side}`] === 0) {
        this[`overlordMultiplier${side}`] = 0.02;
      };
      this[`overlordCount${side}`]++;
    };
    if (itemTitle.includes('Fimbulwinter') || itemTitle.includes(`Winter's Approach`)) {
      if (this[`winterCount${side}`] === 0) {
        this[`winterMultiplier${side}`] = 0.15;
      };
  ...
  onInvenClick = (event, side, i) => {
    ...
    if (itemTitle === `Overlord's Bloodmail`) {
      if (this[`overlordCount${side}`] === 1) {
        this[`overlordMultiplier${side}`] = 0;
      };
      this[`overlordCount${side}`]--;
    };
    if (itemTitle.includes('Fimbulwinter') || itemTitle.includes(`Winter's Approach`)) {
